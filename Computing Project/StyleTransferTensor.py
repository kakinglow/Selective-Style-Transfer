...
save_name = 'stylephoto.jpg'

vgg_weights = "vgg19_weights_tf_dim_ordering_tf_kernels_notop.h5"


# Loads the image and sets a max dimension for gpu purposes
...
# Once image is loaded, it is formated into its appropriate tensor
...
def gram_matrix(input_tensor):
  ...
  # we reshape the tensor to a 2D array of Nc x (Nh*Nw)
...
def compute_loss(model, loss_weights, generated_output_activations, gram_style_features, content_features, num_content_layers, num_style_layers):
  ...
  # The loss layer weighed equally in each contribution
...
def run_style_transfer(content_path, style_path, num_iterations=100, content_weight=0.1, style_weight=0.9): 
  ...
  # weight of each content and style images i.e alpha & beta
  ...
  # Put loss as infinity before training starts and Create a variable to hold best image 
  ...
  for i in range(num_iterations):
    ...
    if (i+1)%100 == 0:
      ...
      output.save('Products/'+str(i+1)+'-'+save_name)
      width, height, _ = best_img.shape
...
#width, height = run_style_transfer(content_path, style_path)
#print(width, height)